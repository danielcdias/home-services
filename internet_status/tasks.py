...
def generate_monthly_summaries():
    """
    Aciona a sumarização mensal através do InternetCleanup.
    """
    from internet_status.services import InternetCleanup
    try:
        InternetCleanup().run_monthly_summarization()
        logger.info("[SCHEDULER] Sumarização mensal concluída com sucesso.")
    except Exception as e:
        logger.error(f"[SCHEDULER] Erro na sumarização mensal: {e}")
